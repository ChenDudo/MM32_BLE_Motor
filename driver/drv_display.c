...
void DRV_Display(u8 led)
{
    ...
    (led & 0x01) ? LD3_on()  : LD3_off();
	(led & 0x02) ? LD4_on()  : LD4_off();
	(led & 0x04) ? LD5_on()  : LD5_off();
	(led & 0x08) ? LD6_on()  : LD6_off();
    (led & 0x10) ? LD7_on()  : LD7_off();
	(led & 0x20) ? LD8_on()  : LD8_off();
	(led & 0x40) ? LD9_on()  : LD9_off();
	(led & 0x80) ? LD10_on() : LD10_off();
#endif
}
